...
int	pars_light(t_pars_state *st, t_scene_parsed *scene, t_elem_role role)
{
	...
	if (!pars_next_tok(st, &token) || scan_point(token,
			parsed_element.u_data.light.position))
		return (ERR_PARS);
	if (!pars_next_tok(st, &token) || scan_float(token,
			&parsed_element.u_data.light.brightness))
		return (ERR_PARS);
	if (parsed_element.u_data.light.brightness < 0.0f
		|| parsed_element.u_data.light.brightness > 1.0f)
		return (ERR_PARS);
	if (!pars_next_tok(st, &token) || scan_color(token,
			parsed_element.u_data.light.rgb))
		return (ERR_PARS);
	if (status != 0)
		return (ERR_PARS);
